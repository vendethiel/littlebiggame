...
void CollisionManager::Resolve()
{
  ...
  {
    if (e->GetCollisionType() == CollisionType::Platform && bb.intersects(e->GetBoundingBox()))
    {
  ...
  for (auto &e : _entities)
  {
    if (bb.intersects(e->GetBoundingBox()) || ((e->GetCollisionType() == CollisionType::Platform || e->GetCollisionType() == CollisionType::Wall) && platformCollider.intersects(e->GetBoundingBox())))
    {
      Collide(e.get());
    }
  }
  ...
  else if (_wall)
  {
    ...
    if (_character.GetY() > 0.f // character is in the air
        && _character.GetVY() == 0.f)
    { // character is not falling/jumping
      // fall off platform
      _character.SetVY(-5.f);
    }
